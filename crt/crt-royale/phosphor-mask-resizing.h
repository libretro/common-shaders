    ...
    static const bool vertical)
{
    ...
    const float2 coord_negative =
        float2(first_texel_tile_uv_wrap_2D < float2(0.0));
...
inline float4 tex2Dlod0try(const sampler2D texture, const float2 tex_uv)
{
    ...
    //  One [slow] workaround is to select the lowest mip level:
        ...
        return tex2Dlod(texture, float4(tex_uv, 0.0, 0.0));
    #else
        #ifdef ANISOTROPIC_RESAMPLING_COMPAT_TEX2DBIAS
            return tex2Dbias(texture, float4(tex_uv, 0.0, -16.0));
        #else
            return tex2D(texture, tex_uv);
        #endif
    #endif
}
...
//  We vectorize the loop to help reduce the Lanczos window's cost.
    ...
    #define VERTICAL_SINC_RESAMPLE_LOOP_BODY                                   \
        ...
        const float3 new_sample0 = tex2Dlod0try(texture,                       \
            float2(tex_uv.x, tex_uv_r.x)).rgb;                                 \
        const float3 new_sample1 = tex2Dlod0try(texture,                       \
            float2(tex_uv.x, tex_uv_r.y)).rgb;                                 \
        const float3 new_sample2 = tex2Dlod0try(texture,                       \
            float2(tex_uv.x, tex_uv_r.z)).rgb;                                 \
        const float3 new_sample3 = tex2Dlod0try(texture,                       \
...
float3 downsample_vertical_sinc_tiled(const sampler2D texture,
    const float2 tex_uv, const float2 texture_size, static const float dr,
...
float3 downsample_horizontal_sinc_tiled(const sampler2D texture,
    ...
    float2 duv_dx, float2 duv_dy)
{
    ...
    const float2 pixel_in_first_half_tile = float2(tile_uv < float2(0.5));
    const float2 jump_exists = float2(duv_dx + duv_dy > float2(0.5));
